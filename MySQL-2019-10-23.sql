...
-- PK로 지정되어 있지만
-- INSERT를 수행할 때 0의 값을 지정해주면
-- 자동으로 값을 생성하여 만들어준다.
INSERT INTO tbl_score(s_id, s_std, s_subject, s_score)
VALUES(0,'홍길동','국어',90);
SELECT * FROM tbl_score; -- 1 홍길동 국어 90

INSERT INTO tbl_score(s_std, s_subject, s_score)
VALUES('홍길동','수학',80); -- 2 홍길동 수학 80

-- INT PK에 auto_increment가 지정되어 있더라도
-- 0 이외의 값을 지정하면
-- 지정한 값이 칼럼에 저장된다.
-- 이 때, PK로 설정된 상태에서 중복값이 발생하여
-- INSERT 오류가 발생할 수 있다.
INSERT INTO tbl_score(s_id, s_std, s_subject, s_score)
VALUES(2, '홍길동','과학',80); -- 오류가 남,,Error Code: 1062. Duplicate entry '2' for key 'PRIMARY'

-- AUTO_INCREMENT로 지정된 칼럼은
-- INT Type으로 지정을 해야하고
-- INSERT를 수행할 때 Projection에 
-- 칼럼을 지정하지 말거나
-- 값을 0으로 설정하여 명령을 수행해야 한다.
SELECT * FROM tbl_score;

INSERT INTO tbl_score(s_id, s_std, s_subject, s_score)
VALUES(3, '홍길동','과학',80); -- 3 홍길동 과학 80

INSERT INTO tbl_score(s_std, s_subject, s_score)
VALUES('홍길동','영어',80); -- 4 홍길동 영어 80

INSERT INTO tbl_score(s_id, s_std, s_subject, s_score)
VALUES(100, '홍길동','영어',80); -- 100 홍길동 영어 80

INSERT INTO tbl_score(s_std, s_subject, s_score)
VALUES('홍길동','영어',80); -- 101 홍길동 영어 80
SELECT * FROM tbl_score;

INSERT INTO tbl_score(s_id, s_std, s_subject, s_score)
VALUES(88, '홍길동','영어',80); -- 88 홍길동 영어 80

INSERT INTO tbl_score(s_std, s_subject, s_score)
VALUES('홍길동','영어',80); -- 102 홍길동 영어 80

-- 100번 이후로 삭제하고 id값 안 넣은채로 다시 추가
DELETE FROM tbl_score
WHERE s_id >= 100;
SELECT * FROM tbl_score;

INSERT INTO tbl_score(s_std, s_subject, s_score)
VALUES('홍길동','미술',80); -- 103 홍길동 미술 80

SELECT * FROM tbl_score;

-- AUTO_INCREMENT로 지정된 칼럼은 한번 최대값이 지정되고나면
-- 중간의 값들을 삭제하더라도 
-- 최대값보다 큰 값들이 생성되어
-- 기존의 값을 재활용하지 않더라
...
-- SELECT를 수행하면
-- 기본값으로 AUTO_INCREMENT로 설정된 PK 순으로
-- 정렬되어 보여진다.

-- ORDER BY
-- 실무에서 많은 TABLE를 JOIN하거나,
-- SUBQUERY를 사용하는 SQL을 작성할 때
...
FROM tbl_score
GROUP BY s_std; -- 홍길동, 490

SELECT s_std,
-- 현재레코드의 s_subject 항목이 국어이면 이 위치에 s_score를 표시하고 그렇지않으면 0을 표시하라
	...
	CASE WHEN s_subject = '과학' 
            ...
            ELSE 0 
	END AS 과학
FROM tbl_score; -- 피벗방식으로 데이터를 옆으로 나열해서 보여줌
-- 홍길동 90 0, 홍길동 0 80, 홍길동 0 80, 홍길동 0 0
...
FROM tbl_score -- SUM없이 여기까지 실행하면 홍길동 90 0 0, 홍길동 0 80 0
GROUP BY s_std; -- 홍길동 90 80 80,, 나열없이 더해서 나옴
-- SUM과 AVG까지 하면,,, 홍길동 90 80 80 490 82

-- 석차추가,, 오라클에선 RANK로 쉽게 했지만 MySQL에선 어려움
SELECT s_std,
	SUM(IF(s_subject = '국어', s_score, 0)) AS 국어,
    SUM(IF(s_subject = '수학', s_score, 0)) AS 수학,
    SUM(IF(s_subject = '과학', s_score, 0)) AS 과학,
...
FROM tbl_score 
GROUP BY s_std; -- 홍길동 90 80 80 490 81.7

-- ROUND : 반올림을 해서 소수점 자릿수 맞추기
-- ROUND(값), ROUND(값,) : 정수형으로
...
SELECT s_std,
    ...
    SUM(IF(s_subject = '과학', s_score, 0)) AS 과학,
    SUM(s_score) AS 총점,
    TRUNCATE(AVG(s_score),1) AS 평균
FROM tbl_score 
GROUP BY s_std; -- 홍길동 90 80 80 490 81.6
...
-- UPDATE, DELETE 는 반드시 WHERE를 사용하자!!
...
SELECT * FROM tbl_score2; -- S0001 B0001 90, S0001 B0002 80 ,,,
...
SELECT * FROM tbl_subject;  -- B0001 국어, B0002 영어, B0003 수학, B0004 과학

-- score와 subject table join
SELECT * FROM tbl_score2 AS SC, tbl_subject AS SB
WHERE SC.s_subject = SB.sb_code; -- S0001 B0001 90 null B0001 국어 null,,, 나열

-- 테이블들의 칼럼이름이 중복되지 않으면
-- Projection을 나열할 때 Alias를 사용하지 않아도 된다.
SELECT SC.s_id, s_std, s_subject, sb_name, s_score 
FROM tbl_score2 AS SC, tbl_subject AS SB
WHERE SC.s_subject = SB.sb_code; -- S0001 B0001 국어 90, S0001 B0002 영어 80,,,

-- LEFT JOIN은 TABLE간에 참조무결성이 보장되지 않을 경우
...
SELECT * FROM view_score; -- 1 S0001 B0001 국어 90, 2 S0001 B0002 영어 80,,,
...
-- MySQL FK를 설정을 할 때
-- SubTable(tbl_score2)의 칼럼 NOT NULL로 설정되어 있어야 한다.
-- MySQL FK로 설정되는 칼럼에 제약조건들이 있다.
-- INT 형일 경우 : NOT NULL DEFAULT = 0
-- 문자열일 경우 VARCHAR() NOT NULL
ALTER TABLE tbl_score2 MODIFY s_subject VARCHAR(5) NOT NULL; -- VARCHAR가 아닌 nVARCHAR로 해서 오류가 났었음
