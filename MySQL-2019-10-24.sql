-- FK, 참조무결성
-- 2개 이상의 테이블을 EQ JOIN을 실행했을 때
-- 연관 정보가 원하는 모든 데이터가
-- 보여진다 라는 조건
-- FK 설정되는 TABLE은 1:N의 관계로 설정
-- 테이블을 FK 지정하여 참조무결성 관계를 설정하겠다
-- 테이블을 생성할 때
-- MySQL의 경우 
-- ENGINE 옵션을 추가해 주는 것이 좋다.
-- character set을 통일시켜주어야 한다.

-- FK 설정하기 위해서
-- 칼럼들은 Type, 크기가 동일하도록 생성되어야 한다.
-- 1:N 일때
-- 1에 해당하는 
...
-- 오라클 : DROP CONSTARINT FKSCORE

-- MYSQL DB 사전으로부터 제약조건 LIST를 확인
SELECT * FROM INFORMATION_SCHEMA.TABLE_CONSTRAINTS
WHERE table_name = 'tbl_score2';

-- tbl_score2와 tbl_subject를 FK로 설정했다
-- 참조무결성관계(Relations Ship) 설정

-- tbl_subject테이블에 아직 등록되지 않은
-- B0100 과목으로 성적을 입력하라고 시도한다.
...
-- FK 조건에 위배되서 데이터를 추가할 수 없다.

-- tbl_subject에 해당 교과목코드의 과목을 먼저 추가해야 한다.
-- Error Code: 1366. Incorrect string value: '\xEC\x9D\xB4\xEB\xAA\xBD...' for column 's_std' at row 1
DELETE FROM tbl_score WHERE s_id = 5;
SELECT * FROM tbl_score2;

-- Error Code: 1452. Cannot add or update a child row: a foreign key constraint fails 
-- (`mydb`.`#sql-688_3`, CONSTRAINT `FK_01` FOREIGN KEY (`s_subject`) REFERENCES `tbl_subject` (`sb_code`))
...
-- 등록하고 났더니 subject의 B0100 콛를 잘못등록했다.
-- B0100이 아니라 B0005로 코드를 변경하고 싶다.
...
-- subject 테이블에만 데이터가 등록된 상태에서는
-- 자유롭게 UPDATE, DELETE가 수행된다.
UPDATE tbl_subject
SET sb_code = 'B2000'
WHERE sb_code = 'B1000';
...
FOREIGN KEY (s_subject)
REFERENCES tbl_subject(sb_code)
ON UPDATE CASCADE -- ON UPDATE CASCADE라는 옵션
ON DELETE CASCADE;
